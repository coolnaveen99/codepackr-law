...
import { Home, Search, Star, MoreHorizontal, BookOpen, Scale, TrendingUp, ClipboardList, GraduationCap } from 'lucide-react';
          ...
          return (
            <button key={tab.id} type="button" onClick={() => onSelectTab(tab.id)}
              ...
              className={`relative flex flex-1 flex-col items-center justify-center gap-0.5 min-w-0 min-h-[44px] px-1 py-1 rounded-xl transition-colors cursor-pointer active:scale-95 ${
                isActive ? 'text-[color:var(--brand,var(--color-accent,#d97706))]' : 'text-[color:var(--ink-muted,var(--color-muted,#64748b))]'
              }`}>
              ...
              {isActive && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-[color:var(--brand,var(--color-accent,#d97706))]" aria-hidden />}
            </button>
          );
        })}
      </div>
