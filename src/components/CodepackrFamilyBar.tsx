...
const ACTIVE_CLASS: Record<FamilyProductId, string> = {
  ...
  study:   "text-indigo-600 font-semibold border-b-2 border-indigo-600 bg-indigo-50/60",
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 h-9 flex items-center justify-between">
        <div className="flex items-center gap-2 font-medium text-slate-700">
          <span className="flex items-center gap-1.5 opacity-90 font-semibold tracking-tight">
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-slate-800 opacity-80" />
              ...
              <li key={item.id} className="h-full flex items-center">
                {isCurrent ? (
                  ...
                  <a
                    ...
                    className="h-full inline-flex items-center gap-1.5 px-2.5 py-0 text-[12px] opacity-85 hover:opacity-100 transition-all rounded-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-500 text-slate-600 hover:text-slate-900 hover:bg-slate-200/50"
                  >
                    {showIcons && <FamilyIcon id={item.id} size={14} />}
                    <span>{labelText}</span>
                  </a>
              ...
              <ul className="py-1">
                {CODEPACKR_FAMILY.map((item) => {
                  ...
                  return (
                    <li key={item.id}>
                      {isCurrent ? (
                        <div className="w-full flex items-center justify-between px-3 py-2.5 min-h-[44px] text-[12px] font-medium bg-slate-100 text-slate-900" aria-current="page">
                          ...
                          <span className="text-[9px] px-1.5 py-0.5 rounded font-mono font-bold uppercase tracking-wider bg-slate-200 text-slate-700">Current</span>
                        </div>
                      ) : (
                        <a
                          href={item.href}
                          target={linkTarget}
                          rel={linkTarget === "_blank" ? "noopener noreferrer" : undefined}
                          className="w-full flex items-center justify-between px-3 py-2.5 min-h-[44px] text-[12px] text-slate-700 hover:text-slate-900 hover:bg-slate-50 transition-colors"
                        >
                          <div className="flex items-center gap-2">
                            <FamilyIcon id={item.id} size={15} />
                            <div>
