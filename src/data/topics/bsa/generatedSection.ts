...
export function createBsaSectionTopic(sectionId: string): TopicContent {
  ...
  return {
    ...
    sections: [
      ...
      {
        id: `bsa-${sectionId}-module-2`,
        title: 'Textual Anatomy, Exceptions and Connected Rules',
        order: 2,
        content: [
          `Deconstruct every clause of BSA s. ${sectionId}; do not replace the enacted rule with a one-line Evidence Act summary.`,
          `Read it with ${relatedText}. Connected provisions may control foundation, exclusion, presumptions, witnesses or the mode of proof.`,
          section?.flags.includes('new') ? 'The catalog marks this as a new BSA provision; do not invent a historical equivalent.' : 'Historical concordance assists comparison only and is not the current citation.',
        ],
      },
      {
        id: `bsa-${sectionId}-module-3`,
      ...
      {
        id: `bsa-${sectionId}-module-5`,
        title: 'Trial Roadmap, Forum, Limitation and Repeal Savings',
        order: 5,
        content: [
          `Apply BSA s. ${sectionId} at the correct stage of trial or inquiry and identify whether the objection concerns tender, marking, admissibility, proof or final weight.`,
    ...
    provisions: [
      { actId: 'bsa', actName: 'Bharatiya Sakshya Adhiniyam, 2023', provisionId: `bsa-${sectionId}`, section: `s. ${sectionId}`, title },
      ...
      { actId: 'bsa', actName: 'Bharatiya Sakshya Adhiniyam, 2023', provisionId: 'bsa-s-104', section: 's. 104', title: 'Burden of proof' },
      { actId: 'bsa', actName: 'Bharatiya Sakshya Adhiniyam, 2023', provisionId: 'bsa-s-109', section: 's. 109', title: 'Burden of proving fact especially within knowledge' },
      { actId: 'bsa', actName: 'Bharatiya Sakshya Adhiniyam, 2023', provisionId: 'bsa-s-170', section: 's. 170', title: 'Repeal and savings' },
    ],
    examples: [
      { id: `bsa-${sectionId}-ill-1`, title: 'Example 1 - Foundation and admissibility satisfied', illustrationType: 'practical', description: `A party identifies the fact in issue, establishes the statutory relevancy link under s. ${sectionId}, proves the source and authenticity of the record, and satisfies the applicable documentary or electronic foundation. The Court may receive the material and then assess its weight.` },
      { id: `bsa-${sectionId}-ill-2`, title: 'Example 2 - Relevancy does not cure a defect', illustrationType: 'fail-scenario', description: `A party offers a relevant screenshot or hearsay statement but cannot satisfy the applicable statutory foundation, certificate, directness or privilege rule. The Court must analyse admissibility separately instead of treating logical relevance as automatic proof.` },
    ],
    hypotheticals: [
      {
        id: `bsa-${sectionId}-hypo`,
        title: `Chamber Practice Hypothetical: BSA s. ${sectionId}`,
        ...
        question: `Whether the material is relevant and admissible under BSA s. ${sectionId}, and what proof and burden consequences follow.`,
        applicableLaw: `BSA s. ${sectionId}, connected provisions ${relatedText}, ss. 3, 55, 57, 63, 104 and 109 where applicable, and s. 170 savings.`,
    ...
    distinctions: [
      {
        id: `bsa-${sectionId}-distinction`,
        title: `BSA s. ${sectionId}: relevancy, admissibility and proof`,
        left: `BSA s. ${sectionId}`,
        right: relatedText,
        rows: [
          { point: 'Function', left: title, right: 'Connected relevancy, foundation, burden or exclusion rule' },
          { point: 'Question', left: 'What fact or evidentiary act does this section govern?', right: 'What additional condition must be satisfied?' },
          { point: 'Trial consequence', left: 'Apply the section to the tendered material', right: 'Then decide admissibility, credibility and weight separately' },
        ],
      },
    ],
    misconceptions: [
      { id: `bsa-${sectionId}-trap-1`, trap: `Citing only the old Evidence Act number for s. ${sectionId}.`, correction: `Use BSA s. ${sectionId} for the current regime and mention the historical number only as concordance or when applying the s. 170 savings rule.` },
      { id: `bsa-${sectionId}-trap-2`, trap: 'Relevancy automatically means admissibility and proof.', correction: 'Relevancy, admissibility, proof and probative weight are separate forensic inquiries.' },
      { id: `bsa-${sectionId}-trap-3`, trap: 'BSA s. 109 reverses the entire burden of proof.', correction: 'The initial burden remains with the party asserting the legal proposition; special knowledge operates only on the facts within that statutory rule.' },
    ],
    cases: [
      {
        ...
        relevance: `Use where s. ${sectionId} concerns electronic records or digital proof; it does not decide the relevancy or admissibility of every non-digital item.`,
      },
      {
        ...
        relevance: `Use only for digital evidence issues connected with s. ${sectionId}; distinguish it from ordinary oral, physical-document or privilege questions.`,
      },
      {
