...
export async function synthesizeCatalogSection(
  ...
  const currentLaw = [
    ...
    p.id === '106' && kind === 'bns'
      ...
      : '',
    kind === 'bnss'
      ? `Transition checkpoint: Section 531(2)(a) preserves the CrPC for appeals, applications, inquiries, trials and investigations pending immediately before 1 July 2024. Identify the procedural date and status before choosing BNSS or CrPC.`
      : '',
  ]
    .filter(Boolean)
    .join(' ')
  ...
  const briefAnswer = [
    ...
    kind === 'bnss'
      ? `Transition and evidence. State whether the proceeding is governed by BNSS or saved CrPC procedure under s. 531. For electronic case records, apply BSA ss. 104–106 and s. 63 only where the record is tendered as evidence; do not treat an evidentiary rule as a substitute for a procedural condition.`
      : '',
  ...
  const submissionsAnswer = [
    ...
    kind === 'bnss'
      ? `Layer 4 — Procedural transition and evidence. Identify the stage of the criminal process, the competent criminal court, the applicable BNSS step, any s. 531 saving, and any BSA s. 63 requirement for digital material.`
      : '',
    `Layer 5 — Application and current-law close. Apply the test to the problem facts in IRAC form (Issue, Rule, Application, Conclusion). Close with: “The governing citation on or after 1 July 2024 is ${meta.short} s. ${p.id}.”`,
  ].join('\n\n')
  ...
  return {
    ...
    examTips: [
      ...
      ...(kind === 'bnss'
        ? ['Check the proceeding date and s. 531 savings before citing BNSS.', 'Keep procedure under BNSS distinct from offences under BNS and proof under BSA.']
        : []),
