...
export function createBnssSectionTopic(sectionId: string): TopicContent {
  ...
  return {
    ...
    sections: [
      ...
      {
        id: `bnss-${sectionId}-module-2`,
        title: 'Textual Anatomy, Provisos and Connected Provisions',
        order: 2,
        content: [
          `Deconstruct every clause of BNSS s. ${sectionId}; do not replace the provision with a CrPC summary.`,
          `Read it with ${relatedText}. A connected section may define the condition, forum, time limit or remedy without replacing this section.`,
          section?.flags.includes('new') ? 'The catalog marks this as a new BNSS provision; do not invent a historical equivalent.' : 'Use historical concordance only to explain development, never as the current citation.',
        ],
      },
      {
        id: `bnss-${sectionId}-module-3`,
        title: 'Mandatory Procedural Ingredients and Stage Test',
        order: 3,
        content: [
          `First ingredient: the proceeding must be at the statutory stage addressed by s. ${sectionId}.`,
      ...
      {
        id: `bnss-${sectionId}-module-5`,
        title: 'Procedural Roadmap, Forum, Limitation and Transition',
        order: 5,
        content: [
          `For proceedings under the post-1 July 2024 regime, cite BNSS s. ${sectionId} and verify the First Schedule or special statute where applicable.`,
    ...
    provisions: [
      { actId: 'bnss', actName: 'Bharatiya Nagarik Suraksha Sanhita, 2023', provisionId: `bnss-${sectionId}`, section: `s. ${sectionId}`, title },
      { actId: 'bsa', actName: 'Bharatiya Sakshya Adhiniyam, 2023', provisionId: 'bsa-s-63', section: 's. 63', title: 'Admissibility of electronic records' },
      { actId: 'bnss', actName: 'Bharatiya Nagarik Suraksha Sanhita, 2023', provisionId: 'bnss-531', section: 's. 531', title: 'Repeal and savings' },
    ],
    examples: [
      { id: `bnss-${sectionId}-ill-1`, title: 'Example 1 - Conditions satisfied', illustrationType: 'practical', description: `A proceeding is at the stage governed by s. ${sectionId}; the authorised officer or court records each statutory reason, follows the required notice and hearing path, and acts within jurisdiction. The order is tested against the section and its connected provisions.` },
      { id: `bnss-${sectionId}-ill-2`, title: 'Example 2 - Boundary defect', illustrationType: 'fail-scenario', description: `The decision-maker applies s. ${sectionId} without checking the procedural stage, the s. 531 savings rule or an express statutory condition. The advocate identifies the missing condition and seeks the precise procedural remedy rather than asserting an automatic result unsupported by the Act.` },
    ],
    hypotheticals: [
      {
        id: `bnss-${sectionId}-hypo`,
        title: `Chamber Practice Hypothetical: BNSS s. ${sectionId}`,
        facts: `A criminal proceeding is challenged after an order applying BNSS s. ${sectionId} to a disputed procedural step. The record contains a disagreement about the commencement status, forum, statutory condition and authenticity of an electronic record.`,
        question: `Whether the order under BNSS s. ${sectionId} is sustainable and what immediate relief is available.`,
        applicableLaw: `BNSS s. ${sectionId}, s. 531, connected provisions ${relatedText}; BSA ss. 63, 104 and 106; Articles 14, 21 and 22.`,
        analysis: `1. Identify the stage and whether the proceeding was pending immediately before 1 July 2024. 2. State the exact test in s. ${sectionId}. 3. Verify jurisdiction, notice, reasons, time limits and hearing requirements. 4. Test the electronic record separately under BSA s. 63. 5. Grant only the remedy supported by the proved defect, such as modification, release, disclosure, remand correction, quashing where maintainable or appellate interference.`,
    ...
    distinctions: [
      {
        id: `bnss-${sectionId}-distinction`,
        title: `BNSS s. ${sectionId} and its connected procedure`,
        left: `BNSS s. ${sectionId}`,
        right: relatedText,
        rows: [
          { point: 'Function', left: title, right: 'Connected stage, condition or remedy' },
          { point: 'Application', left: 'Apply its own enacted ingredients', right: 'Do not import the neighbour\'s test automatically' },
          { point: 'Transition', left: 'Check s. 531 before selecting BNSS', right: 'Historical CrPC numbering is concordance only' },
        ],
      },
    ],
    misconceptions: [
      { id: `bnss-${sectionId}-trap-1`, trap: `Citing only the old CrPC number for s. ${sectionId}.`, correction: `Use BNSS s. ${sectionId} for the current regime and mention the historical number only when the transition or comparison requires it.` },
      { id: `bnss-${sectionId}-trap-2`, trap: 'Treating procedure, offence and proof as one issue.', correction: 'Analyse BNSS procedure, BNS or special-statute liability, and BSA proof as separate legal layers.' },
      { id: `bnss-${sectionId}-trap-3`, trap: 'Using BSA s. 106 to avoid proving foundational facts.', correction: 'The asserting party must first establish the legal and factual foundation; special knowledge does not erase the initial burden.' },
    ],
    cases: [
      {
        ...
        relevance: `Use only where s. ${sectionId} concerns information, investigation or a connected pre-cognizance step; do not treat this case as authority for every BNSS provision.`,
      },
      {
        ...
        relevance: `Use where s. ${sectionId} concerns arrest, custody, remand or liberty; otherwise identify a section-specific authority instead of forcing this case into the argument.`,
      },
      {
        ...
        relevance: `Use where s. ${sectionId} directly engages arrest, custody, production or custody records; do not cite it as a substitute for the statutory text.`,
      },
    ],
    questionsAndAnswers: [
      {
        id: `bnss-${sectionId}-brief`,
        draftingCategory: 'brief',
        question: `Provide a structured legal brief on BNSS s. ${sectionId} - ${title}.`,
        answer: `I. ISSUE AND JURISDICTION\nIdentify the procedural stage, competent forum, commencement status and precise order sought.\n\nII. RULE\nBNSS s. ${sectionId} governs ${title.toLowerCase()}. Read its clauses, provisos and connected provisions ${relatedText}; then test s. 531 savings.\n\nIII. APPLICATION\nMap each fact to jurisdiction, statutory condition, notice, reason, time limit and evidentiary foundation. Test electronic records under BSA s. 63 and burdens under ss. 104-106.\n\nIV. CONCLUSION\nState whether the order stands, must be modified or requires a specific procedural remedy.`,
        explanation: 'IRAC brief with current-law, transition and electronic-evidence checkpoints.',
      },
      {
        id: `bnss-${sectionId}-submissions`,
        draftingCategory: 'submissions',
        question: `Draft written submissions on BNSS s. ${sectionId} - ${title}.`,
        answer: `I. PROCEDURAL PROVENANCE\nSet out the registration date, stage, orders and material relied upon.\n\nII. STATUTORY SCHEME\nDeconstruct BNSS s. ${sectionId}, the Chapter setting, connected provisions ${relatedText} and s. 531 savings.\n\nIII. MANDATORY INGREDIENTS\nAddress forum, jurisdiction, conditions precedent, notice, reasons, time limits, hearing and the statutory consequence.\n\nIV. PRECEDENTS AND EVIDENCE\nUse only verified authorities relevant to the proposition. Apply BSA ss. 63, 104-106 to digital and factual proof without shifting the initial burden improperly.\n\nV. REBUTTAL AND PRAYER\nAnswer the transition, maintainability and merits objections, then seek the precise order supported by the record.`,
        explanation: 'Senior Counsel written-submissions structure for a BNSS section.',
      },
    ],
    bareActPointers: [`BNSS s. ${sectionId}`, 'BNSS s. 531', 'BSA ss. 63, 104-106'],
    examTips: [`Begin with the exact current-law citation: BNSS s. ${sectionId}.`, 'Identify stage, forum, jurisdiction and remedy before discussing merits.', 'Check s. 531 savings for every pre-1 July 2024 pending matter.', 'Keep BNS liability and BSA proof separate from BNSS procedure.'],
    revisionPoints: [`BNSS s. ${sectionId}: ${title}.`, 'List every statutory condition and connected provision.', 'Check the competent court, limitation rule and transition status.', 'Apply BSA s. 63 to electronic records only when they are tendered as evidence.'],
    relatedTopics: related.map((id) => `s-${id}`),
  }
}