...
export default function App() {
  ...
  useEffect(() => {
    if (route.type === 'home') setPageMeta({ title: `${SITE_NAME} — AIBE & Judiciary`, description: SITE_TAGLINE, path: '/' })
    if (route.type === 'subjects') setPageMeta({ title: `All Subjects | ${SITE_NAME}`, description: SITE_TAGLINE, path: '/subjects' })
    ...
    if (route.type === 'topic') {
      ...
      setPageMeta({ title: `${pair?.topic.name ?? 'Topic'} | ${SITE_NAME}`, description: SITE_TAGLINE, path: `/subjects/${route.subjectSlug}/${route.topicId}` })
    }
  }, [route])
    ...
    <div className={dark ? 'dark' : ''}>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-8 paper-grid">
          ...
          {route.type === 'case-law' && <CaseLawLibrary judgmentId={route.judgmentId} onOpenJudgment={(id) => { setCaseLawUrl(id); setRoute({ type: 'case-law', ...(id ? { judgmentId: id } : {}) }) }} onBackToLibrary={() => { setCaseLawUrl(); setRoute({ type: 'case-law' }) }} />}
